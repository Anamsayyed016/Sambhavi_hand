/**
 * Upserts NEW KANCHIPURAM SILK LEHENGA catalog #5 under Lehenga Collection.
...
async function main() {
  ...
  const product = await prisma.product.upsert({
    where: { slug: 'kanchipuram-silk-lehenga-05' },
    create: {
      slug: 'kanchipuram-silk-lehenga-05',
      sku: 'SH-KANCHIPURAM-SILK-LEHENGA-05',
  ...
  console.log(
    `KANCHIPURAM SILK LEHENGA #5 ready: ${product.id} (${product.slug}) ₹${product.price}`,
  )
}
