/**
 * Upserts NEW KANCHIPURAM SILK LEHENGA catalog #4 under Lehenga Collection.
...
async function main() {
  ...
  const product = await prisma.product.upsert({
    where: { slug: 'kanchipuram-silk-lehenga-04' },
    create: {
      slug: 'kanchipuram-silk-lehenga-04',
      sku: 'SH-KANCHIPURAM-SILK-LEHENGA-04',
  ...
  console.log(
    `KANCHIPURAM SILK LEHENGA #4 ready: ${product.id} (${product.slug}) ₹${product.price}`,
  )
}
