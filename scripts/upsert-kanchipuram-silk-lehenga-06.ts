/**
 * Upserts NEW KANCHIPURAM SILK LEHENGA catalog #6 under Lehenga Collection.
...
async function main() {
  ...
  const product = await prisma.product.upsert({
    where: { slug: 'kanchipuram-silk-lehenga-06' },
    create: {
      slug: 'kanchipuram-silk-lehenga-06',
      sku: 'SH-KANCHIPURAM-SILK-LEHENGA-06',
  ...
  console.log(
    `KANCHIPURAM SILK LEHENGA #6 ready: ${product.id} (${product.slug}) ₹${product.price}`,
  )
}
